...
openAdsterraPop();
// 🛑 Slight delay so ad loads before modal takes focus (but short)
...
if (!embedUrl){
pWrap.innerHTML = `<div style="padding:80px 20px;text-align:center;color:var(--muted)">   Preview not available for embed. Please use the 'Open in Player' button.   </div>`;
...
// we listen for the first real user gesture (click/touch) and then allow an initial pop.
...
try {
userInteracted = true;
// Fire an initial controlled pop shortly after the first interaction (if not already fired)
if (!initialPopFired) {
// small delay to let the user's click complete but still be considered a gesture
setTimeout(()=> {
openAdsterraPop();
}, 150); // 150ms is short but usually within gesture window
}
