...
if (persistentAd) {
persistentAd.innerHTML =     <span class="ad-label">Sponsored</span>     ${ADSTERRA_NATIVE_BANNER_SCRIPT}    ;
}
...
const bannerAd = modal.querySelector('.adsterra-banner-placement');
if (bannerAd) bannerAd.innerHTML =     <script type="text/javascript">     atOptions = {     'key' : 'd1be46ed95d3e2db572824c531da5082',     'format' : 'iframe',     'height' : 90,     'width' : 728,     'params' : {}     };     </script>    ;
const socialBarAd = modal.querySelector('.adsterra-socialbar-placement');
...
// Convert Streamtape /v/ link to /e/ embed link for the dedicated player    
  if(targetUrl.includes("/v/")) {    
    const idMatch = targetUrl.match(/\/v\/([0-9A-Za-z_-]+)/);    
    if(idMatch && idMatch[1]) {    
      finalWatchUrl = `https://streamtape.com/e/${idMatch[1]}/`;    
    }    
  } else if (!targetUrl.includes("/e/")) {  
    // अगर यह न /v/ है और न /e/ है, तो raw URL का उपयोग करें (fallback)  
  }  

  // watch.html पर embed URL भेजें  
...
// ****** SEO CANONICAL FIX: ****** const currentUrl = window.location.origin + window.location.pathname;
